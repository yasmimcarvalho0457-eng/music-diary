...
function startPresenceListener() {
  ...
  unsubscribePresence = onSnapshot(collection(db, "presence"), () => {
    // Recarrega todo o mapa com os dados recebidos
  });

  unsubscribePresence();

  unsubscribePresence = onSnapshot(collection(db, "presence"), async (snapshot) => {
    presenceMap = new Map();

    snapshot.forEach((docSnap) => {
      presenceMap.set(docSnap.id, docSnap.data());
    });
...
async function renderUsers() {
  ...
  for (const user of uniqueUsers) {
    ...
    const onlineNow = isReallyOnline(user.uid);
...
function updateOnlineStatusOnly() {
  document.querySelectorAll(".chat-user-card").forEach((card) => {
    ...
    const onlineNow = isReallyOnline(uid);
...
function isReallyOnline(uid) {
  const presence = presenceMap.get(uid);

  if (!presence?.online || !presence?.lastSeen?.toDate) return false;

  const seconds = (Date.now() - presence.lastSeen.toDate().getTime()) / 1000;
  return seconds < 120;
}
...
async function getLastTime(otherUid) {
  ...
  return snap.data().updatedAt.toDate().toLocaleTimeString("pt-BR", {
    hour: "2-digit",
    minute: "2-digit"
  });
}
...
function renderChatTitle() {
  ...
  const onlineNow = isReallyOnline(selectedUser.uid);
...
async function createMessageNotification(text, imageUrl) {
  if (!selectedUser) return;

  const preview = imageUrl && !text ? "📷 enviou uma imagem" : text;

  await addDoc(collection(db, "users", selectedUser.uid, "notifications"), {
    type: "message",
    title: "Nova mensagem",
    message: `@${currentProfile.username || "usuaria"}: ${preview}`,
...
messageForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    await createMessageNotification(text, imageUrl);
